...
public:
    int n, m;
    std::vector<FLTARY> w, dw;
    FLTARY b, db, top, grads;
    ...
    void backward_pass(FLTARY top_grads, FLTARY bottom, double momentum, double base_lr) {
        grads.assign(m, 0);
        for(int i = 0; i < n; ++i) {
            for(int j = 0; j < m; ++j) {
                grads[j] += top_grads[i] * w[i][j];
                dw[i][j] = momentum * dw[i][j] + base_lr * top_grads[i] * bottom[j];
                w[i][j] -= dw[i][j];
            }
            db[i] = momentum * db[i] + base_lr * top_grads[i];
            b[i] -= db[i];
        }
...
int main() {
    ...
    double base_lr = 0.01, momentum = 0.5;
...
//	LoadData(std::cin, &nImgRows, &nImgCols, trainImgs, trainLabels, testImgs);
    ...
    for(int i = 0; i < 10000 / trainImgs.size(); ++i) {//max_iter
        for(int j = 0; j < trainImgs.size(); ++j) {//max_iter
            ...
            fc2.backward_pass(loss.grads, relu.top, momentum, base_lr);
            relu.backward_pass(fc2.grads);
            fc1.backward_pass(relu.grads, trainImgs[j], momentum, base_lr);
        }
        std::cout << i;
        tp = fp = 0;
